# I think of myself as a Knight

<div class="fullwidth">
  <div class="fillable"><b>Name</b>:</div>
  <div class="fillable"><b>Pronouns</b>:</div>
</div>
<br>

## Seemings
<br>
<dl>
  <dt>Presentation</dt>
  <dd>manly, feminine, a bit of both, not much of either, as I see fit, elsewise</dd>
  <dt>Costume</dt>
  <dd>shirtwaist ensemble, bicycle suit, cutaway frock, vest and trousers, edwardian gown, duster, ethnic traditional (mexican, native american, oriental, etc)</dd>
  <dt>Plays with</dt>
  <dd>certainty, care, dignity, fervor, ill portent, understanding, ease, agitation</dd>
  <dt>Your armor</dt>
  <dd>suit of platemail, colorful brigandine, breastplate and morion, stitched linen, purely ornamental, faith, virtue</dd>
</dl>
<br>

## Anchors

<ul class="anchor-list">
  <li class="anchor-li">Place yourself in harm's way.</li>
  <li class="anchor-li">Protect someone weaker than yourself.</li>
  <li class="anchor-li">Intervene where honor or valor demands.</li>
  <li class="anchor-li">Adhere to- and demand of others- an excess of decorum.</li>
  <li class="anchor-li">Prove to a ward how foolish they are.</li>
</ul>

## Plays

#### Implements of valor

You enter every pocket equipped with your Sword and Shield.

Your shield can block any problem that comes your way (attacks, dangers, accusations, seductions, etc), just say, **"No `X` may ever harm me!"**, and increase instability by `2`.

Your sword can cut down any obstacle that stands in your way (foes, fiends, walls, illusions, lies, fear, etc), just say, **"No `X` may stand in my way!"**, and increase instability by `3`.

#### What is a knight with no dragon to slay?

**When you conjure up a playbook,** roll.  On 2, you come up with something that both matches the theme and has interesting mechanics. On 1, it either matches the theme or has interesting mechanics, pick one.  On a miss, it's not working out.  Scrap it and start over again.

## Burdens

The first time you gain each burden, fill out the blank with a virtue. While that burden is marked you are not allowed to act in any way that compromises your virtue. When you get **Overburdened**, you may erase one of your virtues, and rewrite it the next time you're burdened.

<dl class="selectable">
  <dt>Psyche</dt>
  <dd>
  Your shield is: flimsy, fragile, a shining mirror, a hungering mouth, wails in pain when used, enormous and seeks to block both good and bad.

  I must never allow someone else to <span class="blank"></span><span class="blank"></span>.
  </dd>
  <dt>Heart</dt>
  <dd>
  When people look at you, they see: a statue, stoic and unfeeling; a champion, who fulfills and rescues; a warrior, terrible and unflinching; a legacy, outdated and pointless; someone playing make-believe, idyllic and daft.

  When asked for it, I must always give up my <span class="blank"></span><span class="blank"></span>.
  </dd>
  <dt>Hands</dt>
  <dd>
  Your sword is: dulled, broken, wreathed in flame, too heavy, smaller than a quill, a simple stick, yearns to cut and rend.

  I must never allow myself to <span class="blank"></span><span class="blank"></span>.
  </dd>
  <dt>Tongue</dt>
  <dd>
  Your oath is: boisterous and full of wind, meek, one more shackle to bear, heavy as gold, betrays your exhaustion, promises more than it can fulfill.

  I must never experience the pleasure of <span class="blank"></span><span class="blank"></span>.
  </dd>
  <dt>Bones</dt>
  <dd>
  Your armor is: tarnished, dented, rusted, radiant and blinding, scalding hot, weeping, covered in spines, beastly.

  No harm or insult may ever befall <span class="blank"></span><span class="blank"></span>.
  </dd>
</dl>

### Your burdens overwhelm you: 

**When you would mark a sixth burden,** don't. Your burdens overwhelm you.

<br>

---
